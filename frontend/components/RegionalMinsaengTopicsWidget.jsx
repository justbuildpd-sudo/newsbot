  ...
  useEffect(() => {
    const loadMinsaengData = async () => {
      try {
        ...
        // 향상된 API 엔드포인트에서 로드
        const response = await fetch('/api/regional-minsaeng-topics?limit=100');
        ...
        if (data.success) {
          ...
          setMinsaengData({
            ...
            total_regions: Object.keys(data.data).length,
            ...
            regional_data: data.data
          });
        } else {
          // API 실패시 기본 데이터 사용
          console.warn('API 로드 실패, 기본 데이터 사용');
          setMinsaengData({
            last_updated: new Date().toISOString(),
            total_regions: 207,
            minsaeng_topic_categories: {
