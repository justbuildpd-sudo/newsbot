...
"""
대규모 통합 시스템
현직 국회의원 + 22대 출마자 + 지방의회의원 + 교육감 + 기초의회의원(5,318명)을
모두 통합하여 완전한 정치인 데이터베이스를 구축합니다.
"""
...
class MassiveIntegrationSystem:
    """대규모 통합 시스템 클래스"""
    ...
    def normalize_politician_data(self, politician: Dict, category: str) -> Dict:
        ...
        normalized = {
            ...
            'election_data': {
                ...
                'election_info': {
                    'election_name': politician.get('election_info', {}).get('election_name'),
                    'election_day': politician.get('election_info', {}).get('election_day'),
            ...
            # 검색 키워드 (최소화)
            'search_keywords': [name, politician.get('party', ''), category],
    ...
    def _get_search_priority(self, category: str, politician: Dict) -> int:
        ...
        priorities = {
            ...
            'basic_council_candidate': 55 if politician.get('is_elected', False) else 40  # 새로 추가
        }
    ...
    def build_massive_search_index(self) -> Dict:
        """대규모 검색 인덱스를 구축합니다."""
        logger.info("🔄 대규모 검색 인덱스 구축 시작...")
        ...
        for politician in self.integrated_politicians:
            ...
            if name:
                ...
                # 메모리 효율성을 위해 상위 3개만 유지
                search_index['by_name'][name_key].sort(
                    key=lambda x: x['political_profile']['search_priority'], 
                    reverse=True
                )
                search_index['by_name'][name_key] = search_index['by_name'][name_key][:3]
            ...
            # 메모리 관리 (1000명마다 가비지 컬렉션)
            if processed_count % 1000 == 0:
                gc.collect()
                logger.info(f"   검색 인덱스 처리: {processed_count:,}명 완료")
        
        logger.info("✅ 대규모 검색 인덱스 구축 완료")
        return search_index
    
    def run_massive_integration(self) -> Dict:
        """대규모 통합 작업을 실행합니다."""
        logger.info("🚀 대규모 통합 시스템 시작")
        ...
        # 2. 데이터 표준화 및 통합 (배치 처리)
        logger.info("🔄 대규모 데이터 표준화 및 통합")
        ...
        for i, candidate in enumerate(self.election_candidates):
            ...
            if (i + 1) % batch_size == 0:
                gc.collect()
                logger.info(f"   22대 출마자 처리: {i + 1:,}명 완료")
        ...
        # 4. 대규모 검색 인덱스 구축
        self.search_index = self.build_massive_search_index()
        ...
        logger.info("✅ 대규모 통합 완료")
        return self.integration_results
    
    def generate_massive_report(self) -> str:
        """대규모 통합 결과 리포트를 생성합니다."""
        stats = self.integration_results['statistics']
        categories = self.integration_results['categories']
        
        report = f"""
🏛️ NewsBot.kr 대규모 통합 시스템 결과
{'='*90}
...
- 🏢 기초의원 출마자: {stats['basic_council_candidates']:,}명
...
"""
    ...
    def save_massive_results(self):
        """대규모 통합 결과를 저장합니다."""
        try:
            logger.info("🔄 대규모 결과 파일 저장 시작...")
            
            # 전체 통합 결과 (압축 저장)
            with open("newsbot_massive_integration.json", 'w', encoding='utf-8') as f:
                json.dump(self.integration_results, f, ensure_ascii=False, indent=1)  # indent 최소화
                ...
                'politicians': [
                    {
                        ...
                        'search_priority': p['political_profile']['search_priority']
                    }
            ...
            with open("newsbot_massive_search.json", 'w', encoding='utf-8') as f:
                json.dump(search_data, f, ensure_ascii=False, indent=1)
            
            # 카테고리별 분리 저장 (메모리 효율성)
            for category in ['current_member', 'election_candidate', 'education_candidate', 'local_politician', 'basic_council_candidate']:
                category_data = [p for p in self.integrated_politicians if p['category'] == category]
                
                with open(f"newsbot_{category}_data.json", 'w', encoding='utf-8') as f:
                    json.dump(category_data, f, ensure_ascii=False, indent=1)
                
                logger.info(f"✅ {category} 데이터 저장 완료: {len(category_data):,}명")
            
            logger.info("✅ 대규모 통합 결과 파일 저장 완료")
...
def main():
    """메인 실행 함수"""
    logger.info("=== NewsBot.kr 대규모 통합 시스템 시작 ===")
    
    integration_system = MassiveIntegrationSystem()
    
    # 대규모 통합 실행
    results = integration_system.run_massive_integration()
    
    if not results:
        logger.error("❌ 대규모 통합 시스템 실행 실패")
        return False
    
    # 결과 저장
    integration_system.save_massive_results()
    
    # 리포트 생성 및 출력
    report = integration_system.generate_massive_report()
    print(report)
    
    # 리포트 파일 저장
    with open("newsbot_massive_integration_report.txt", "w", encoding="utf-8") as f:
        f.write(report)
    
    logger.info("✅ NewsBot.kr 대규모 통합 시스템 완료")
    return True

if __name__ == "__main__":
    success = main()
    if success:
        logger.info("✅ 대규모 통합 시스템 실행 완료")
    else:
        logger.error("❌ 대규모 통합 시스템 실행 실패")
