...
from assembly_members_clean import get_assembly_members, update_mention_count
...
class NewsService:
    ...
    def analyze_politician_mentions(self, news_articles: List[Dict]) -> Dict:
        """뉴스 기사에서 정치인 언급 분석"""
        ...
        for politician in politicians:
            mention_counts[politician['id']] = 0
        ...
        for article in news_articles:
            ...
            for politician in politicians:
                ...
                # 정확한 이름 매칭 (부분 문자열 포함)
                if name in content:
                    mention_counts[politician['id']] += 1
        
        # 언급 횟수 업데이트
        for politician_id, count in mention_counts.items():
            if count > 0:
                update_mention_count(politician_id, count)
        
        return {
            "total_articles": len(news_articles),
            "mention_counts": mention_counts,
            "updated_politicians": [pid for pid, count in mention_counts.items() if count > 0]
        }
    
    def get_political_news_with_mentions(self) -> List[Dict]:
        """정치 뉴스 가져오기 + 정치인 언급 분석"""
        # 기존 정치 뉴스 가져오기
        political_news = self.get_political_news()
        
        # 정치인 언급 분석
        mention_analysis = self.analyze_politician_mentions(political_news)
        
        # 뉴스에 언급 분석 결과 추가
        for article in political_news:
            article['mention_analysis'] = mention_analysis
