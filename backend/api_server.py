...
@app.get("/api/news/with-mentions")
async def get_news_with_mentions():
    """정치인 언급 분석이 포함된 뉴스 가져오기"""
    try:
        news = news_service.get_political_news_with_mentions()
        return {
            "success": True,
            "data": news,
            "count": len(news),
        ...
        raise HTTPException(status_code=500, detail=f"언급 분석 뉴스 조회 실패: {str(e)}")

@app.get("/api/news/analyze-mentions")
async def analyze_news_mentions():
    """현재 뉴스에서 정치인 언급 분석"""
    try:
        news = news_service.get_cached_news()
        mention_analysis = news_service.analyze_politician_mentions(news)
        return {
            "success": True,
            "analysis": mention_analysis,
            "timestamp": datetime.now().isoformat()
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"언급 분석 실패: {str(e)}")
