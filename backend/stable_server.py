#!/usr/bin/env python3
"""
NewsBot 안정 서버 - 맥 환경 최적화
기존 simple_server.py를 기반으로 모든 기능을 통합합니다.
"""
...
app = FastAPI(
    title="NewsBot 안정 서버",
    description="정치인 뉴스 분석 및 평가 시스템 - 맥 최적화",
    version="2.0.0"
)
...
class NewsBotStableService:
    def __init__(self):
        self.politicians_data = {}
        self.db_path = "newsbot_stable.db"
    ...
    def load_politicians(self):
        ...
        try:
            ...
            logger.info(f"✅ 데이터 로드 완료: {len(self.politicians_data)}명")
...
@app.get("/politician.html")
async def politician_page():
    """정치인 분석 페이지"""
    return FileResponse("../politician.html")
